...
def create_profile_curve_from_data(
    name, profile_data, segment_length=1.0, base_radius=1.0
):
    """
...
def format_profile_data_as_code(profile_data, variable_name="PROFILE_DATA"):
    ...
    for i, point in enumerate(profile_data["points"]):
        ...
        lines.append(
            f'            "handle_right_type": "{point["handle_right_type"]}",'
        )
...
def get_default_profile_curve_from_data(
    scene,
    ...
    if not y_profile_obj:
        y_profile_name = f"{segment_type.value}_{profile_type.value}"
        if y_profile_name in bpy.data.objects:
            ...
            y_profile_obj = get_default_profile_curve(
                segment_type, profile_type, segment_length, base_radius
            )
            bpy.context.scene.collection.objects.link(y_profile_obj)

    return y_profile_obj


def get_default_profile_curve(
    segment_type: SegmentType,
    profile_type: ProfileType,
    segment_length=1.0,
    base_radius=1.0,
    ...
    return create_profile_curve_from_data(
        name, profile_data, segment_length, base_radius
    )
...
def normalize_profile_curve(curve_object, segment_length=1.0):
    ...
    for point in profile_data["points"]:
        ...
        handle_right = list(point["handle_right"])
        handle_right[2] = (
            handle_right[2] / segment_length if segment_length > 0 else handle_right[2]
        )
        point["handle_right"] = tuple(handle_right)
