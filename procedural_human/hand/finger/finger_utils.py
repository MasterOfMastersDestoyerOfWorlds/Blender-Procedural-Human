...
def add_finger_armature_to_object(
    finger_object,
    ...
    # Create armature
    armature = create_finger_armature(
        finger_object, num_segments, segment_lengths, curl_direction
    )

    # Create animation if requested
    if create_animation:
        from . import finger_animation as animation
    ...
    for vert_idx, vert in enumerate(mesh.vertices):
        ...
        if total_weight > 0:
            ...
            nearest_seg = min(
                range(num_segments),
                key=lambda i: abs(
                    vert_z - (sum(segment_lengths[:i]) + segment_lengths[i] / 2)
                ),
            )
