...
def create_geometry_nodes_modifier(box: UILayout, context: Context, name: str) -> None:
    ...
    box.operator(f"mesh.procedural_create_{snake_name}", text=f"Create {name}")
    box.operator(f"mesh.procedural_realize_{snake_name}_geometry", text=f"Realize Geometry")
    box.separator()
