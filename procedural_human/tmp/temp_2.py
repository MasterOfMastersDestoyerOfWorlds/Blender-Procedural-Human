...
def create_closest_index_points_group():
    ...
    geometry_proximity.location = (-1150.0, 0.0)
    ...
    viewer_001.label = ""
    viewer_001.location = (-900.0, 150.0)
...
def create_get_index_offset_group():
    ...
    group_input.name = "Group Input"
    group_input.label = ""
    group_input.location = (-1379.740966796875, 41.771934509277344)
    group_input.bl_label = "Group Input"
    # Links for group_input
    ...
    closest_index_points.location = (-1129.740966796875, 191.77194213867188)
    ...
    integer_math_001.name = "Integer Math.001"
    integer_math_001.label = ""
    integer_math_001.location = (-490.25286865234375, -66.2107162475586)
    integer_math_001.bl_label = "Integer Math"
    ...
    closest_index_points_1.location = (-1107.85498046875, -58.8636474609375)
    ...
    integer_math_002.location = (-879.740966796875, 116.77193450927734)
...
def create_mix_angle_group():
    ...
    links.new(sample_index.outputs[0], math.inputs[0])
    ...
    links.new(sample_index_001.outputs[0], math.inputs[1])
...
def create_male_torso__loft_0__group_group():
    ...
    group_output.name = "Group Output"
    group_output.label = ""
    group_output.location = (1313.3272705078125, 815.7308959960938)
    group_output.bl_label = "Group Output"
    group_output.is_active_output = True
    # Links for group_output
    ...
    index_001.label = ""
    index_001.location = (-1229.253173828125, -157.142578125)
    ...
    sample_curve_distances.location = (-895.7106323242188, 219.6610107421875)
    ...
    sample_curve_distances_1.location = (-916.64990234375, -208.04469299316406)
    ...
    integer.label = ""
    integer.location = (-1248.943603515625, 247.13401794433594)
    ...
    spherical_coords.label = ""
    spherical_coords.location = (117.65666961669922, -242.61538696289062)
    ...
    construct_vector.location = (789.223876953125, 42.02174377441406)
    construct_vector.node_tree = create_construct_vector_group()
    construct_vector.bl_label = "Group"
    # Links for construct_vector
    links.new(spherical_coords.outputs[0], construct_vector.inputs[0])
    ...
    spherical_coords_1.label = ""
    spherical_coords_1.location = (86.32595825195312, 241.0050811767578)
    ...
    value.name = "Value"
    value.label = ""
    value.location = (-270.0000915527344, -297.0960693359375)
    ...
    tri_line.label = ""
    tri_line.location = (1361.538818359375, 306.6200256347656)
    ...
    links.new(tri_line.outputs[0], viewer_002.inputs[0])

    tri_line_1 = nodes.new("GeometryNodeGroup")
    tri_line_1.name = "Group.009"
    tri_line_1.label = ""
    tri_line_1.location = (1282.5460205078125, -94.57290649414062)
    ...
    mix_sample.name = "Group.010"
    mix_sample.label = ""
    mix_sample.location = (467.2660217285156, 311.3876037597656)
    mix_sample.node_tree = create_mix_sample_group()
    mix_sample.bl_label = "Group"
    # Links for mix_sample
    links.new(mix_sample.outputs[0], construct_vector.inputs[1])
    links.new(value.outputs[0], mix_sample.inputs[0])
    links.new(spherical_coords.outputs[1], mix_sample.inputs[5])
    links.new(spherical_coords_1.outputs[1], mix_sample.inputs[2])

    mix_sample_1 = nodes.new("GeometryNodeGroup")
    mix_sample_1.name = "Group.011"
    mix_sample_1.label = ""
    mix_sample_1.location = (452.8452453613281, 47.11560821533203)
    mix_sample_1.node_tree = create_mix_sample_group()
    mix_sample_1.bl_label = "Group"
    # Links for mix_sample_1
    links.new(value.outputs[0], mix_sample_1.inputs[0])
    links.new(spherical_coords_1.outputs[2], mix_sample_1.inputs[2])
    links.new(spherical_coords.outputs[2], mix_sample_1.inputs[5])

    mix_sample_2 = nodes.new("GeometryNodeGroup")
    mix_sample_2.name = "Group.012"
    mix_sample_2.label = ""
    mix_sample_2.location = (460.3246765136719, -165.6611785888672)
    mix_sample_2.node_tree = create_mix_sample_group()
    mix_sample_2.bl_label = "Group"
    # Links for mix_sample_2
    links.new(value.outputs[0], mix_sample_2.inputs[0])
    links.new(mix_sample_2.outputs[0], construct_vector.inputs[5])
    links.new(spherical_coords_1.outputs[5], mix_sample_2.inputs[2])
    links.new(spherical_coords.outputs[5], mix_sample_2.inputs[5])
    ...
    loop_normal_vector.location = (-329.6462707519531, -473.0057678222656)
    ...
    loop_normal_vector_1.label = ""
    loop_normal_vector_1.location = (-347.20245361328125, 328.463623046875)
    ...
    vector_math_001.name = "Vector Math.001"
    vector_math_001.label = ""
    vector_math_001.location = (-143.87762451171875, -389.2610168457031)
    vector_math_001.bl_label = "Vector Math"
    ...
    perp_vectors_simple.label = ""
    perp_vectors_simple.location = (466.2374572753906, -462.20556640625)
    perp_vectors_simple.node_tree = create_perp_vectors_simple_group()
    perp_vectors_simple.bl_label = "Group"
    # Links for perp_vectors_simple
    links.new(spherical_coords.outputs[0], perp_vectors_simple.inputs[1])
    ...
    get_index_offset.location = (-627.639892578125, 102.60733032226562)
    ...
    links.new(sample_curve_distances.outputs[1], get_index_offset.inputs[0])
    links.new(sample_curve_distances_1.outputs[1], get_index_offset.inputs[1])
    ...
    links.new(get_index_offset.outputs[5], mix_sample_2.inputs[6])
    links.new(get_index_offset.outputs[2], mix_sample_2.inputs[3])
    links.new(get_index_offset.outputs[5], mix_sample_1.inputs[6])
    links.new(get_index_offset.outputs[2], mix_sample_1.inputs[3])
    ...
    links.new(get_index_offset.outputs[0], mix_sample_1.inputs[1])
    links.new(get_index_offset.outputs[3], set_position.inputs[0])
    links.new(get_index_offset.outputs[3], mix_sample_1.inputs[4])
    links.new(get_index_offset.outputs[3], mix_sample.inputs[4])
    links.new(get_index_offset.outputs[0], mix_sample.inputs[1])
    links.new(get_index_offset.outputs[0], mix_sample_2.inputs[1])
    links.new(get_index_offset.outputs[3], mix_sample_2.inputs[4])
    ...
    viewer_001.name = "Viewer.001"
    viewer_001.label = ""
    viewer_001.location = (-343.2969665527344, 12.024185180664062)
    viewer_001.bl_label = "Viewer"
    viewer_001.ui_shortcut = 0
    ...
    mix_angle.label = ""
    mix_angle.location = (771.568115234375, -291.38116455078125)
    ...
    links.new(value.outputs[0], mix_angle.inputs[0])
