...
@geo_node_group
def create_mix_long_angle_group():
    group_name = "MixLongAngle"
    if group_name in bpy.data.node_groups:
        return bpy.data.node_groups[group_name]
    ...
    group_output.name = "Group Output"
    group_output.label = ""
    group_output.location = (800.0, 0.0)
    group_output.bl_label = "Group Output"
    group_output.is_active_output = True
    ...
    # Sample AngleA from CurveA
    sample_index_a = nodes.new("GeometryNodeSampleIndex")
    sample_index_a.name = "Sample Index A"
    sample_index_a.label = "Sample θ_A"
    sample_index_a.location = (-1100.0, 150.0)
    sample_index_a.bl_label = "Sample Index"
    sample_index_a.data_type = "FLOAT"
    sample_index_a.domain = "POINT"
    sample_index_a.clamp = False
    links.new(group_input.outputs[1], sample_index_a.inputs[0])  # CurveA
    links.new(group_input.outputs[2], sample_index_a.inputs[1])  # AngleA field
    links.new(group_input.outputs[3], sample_index_a.inputs[2])  # Index

    # Sample AngleB from CurveB
    sample_index_b = nodes.new("GeometryNodeSampleIndex")
    sample_index_b.name = "Sample Index B"
    sample_index_b.label = "Sample θ_B"
    sample_index_b.location = (-1100.0, -150.0)
    sample_index_b.bl_label = "Sample Index"
    sample_index_b.data_type = "FLOAT"
    sample_index_b.domain = "POINT"
    sample_index_b.clamp = False
    links.new(group_input.outputs[4], sample_index_b.inputs[0])  # CurveB
    links.new(group_input.outputs[5], sample_index_b.inputs[1])  # AngleB field
    links.new(group_input.outputs[6], sample_index_b.inputs[2])  # Index

    # Step 1: diff = θ_B - θ_A
    math_diff = nodes.new("ShaderNodeMath")
    math_diff.name = "Math Diff"
    math_diff.label = "θ_B - θ_A"
    math_diff.location = (-850.0, 0.0)
    math_diff.bl_label = "Math"
    math_diff.operation = "SUBTRACT"
    math_diff.use_clamp = False
    links.new(sample_index_b.outputs[0], math_diff.inputs[0])
    links.new(sample_index_a.outputs[0], math_diff.inputs[1])

    # Step 2: diff + π
    math_add_pi = nodes.new("ShaderNodeMath")
    math_add_pi.name = "Math Add Pi"
    math_add_pi.label = "+ π"
    math_add_pi.location = (-650.0, 0.0)
    math_add_pi.bl_label = "Math"
    math_add_pi.operation = "ADD"
    math_add_pi.use_clamp = False
    math_add_pi.inputs[1].default_value = 3.1415927410125732
    links.new(math_diff.outputs[0], math_add_pi.inputs[0])

    # Step 3: (diff + π) % 2π
    math_mod_2pi = nodes.new("ShaderNodeMath")
    math_mod_2pi.name = "Math Mod 2Pi"
    math_mod_2pi.label = "% 2π"
    math_mod_2pi.location = (-450.0, 0.0)
    math_mod_2pi.bl_label = "Math"
    math_mod_2pi.operation = "FLOORED_MODULO"
    math_mod_2pi.use_clamp = False
    math_mod_2pi.inputs[1].default_value = 6.2831854820251465
    links.new(math_add_pi.outputs[0], math_mod_2pi.inputs[0])

    # Step 4: wrapped - π = Δ (wrapped difference in [-π, π])
    math_sub_pi = nodes.new("ShaderNodeMath")
    math_sub_pi.name = "Math Sub Pi"
    math_sub_pi.label = "Δ = wrapped - π"
    math_sub_pi.location = (-250.0, 0.0)
    math_sub_pi.bl_label = "Math"
    math_sub_pi.operation = "SUBTRACT"
    math_sub_pi.use_clamp = False
    math_sub_pi.inputs[1].default_value = 3.1415927410125732
    links.new(math_mod_2pi.outputs[0], math_sub_pi.inputs[0])

    # Step 5: Δ > 0 (gives 1.0 or 0.0)
    math_greater = nodes.new("ShaderNodeMath")
    math_greater.name = "Math Greater"
    math_greater.label = "Δ > 0"
    math_greater.location = (-50.0, -150.0)
    math_greater.bl_label = "Math"
    math_greater.operation = "GREATER_THAN"
    math_greater.use_clamp = False
    math_greater.inputs[1].default_value = 0.0
    links.new(math_sub_pi.outputs[0], math_greater.inputs[0])

    # Step 6: * 2.0
    math_times_2 = nodes.new("ShaderNodeMath")
    math_times_2.name = "Math Times 2"
    math_times_2.label = "* 2"
    math_times_2.location = (150.0, -150.0)
    math_times_2.bl_label = "Math"
    math_times_2.operation = "MULTIPLY"
    math_times_2.use_clamp = False
    math_times_2.inputs[1].default_value = 2.0
    links.new(math_greater.outputs[0], math_times_2.inputs[0])

    # Step 7: - 1.0 → sign(Δ) = +1 or -1
    math_sign = nodes.new("ShaderNodeMath")
    math_sign.name = "Math Sign"
    math_sign.label = "sign(Δ)"
    math_sign.location = (350.0, -150.0)
    math_sign.bl_label = "Math"
    math_sign.operation = "SUBTRACT"
    math_sign.use_clamp = False
    math_sign.inputs[1].default_value = 1.0
    links.new(math_times_2.outputs[0], math_sign.inputs[0])

    # Step 8: sign(Δ) * 2π
    math_sign_times_2pi = nodes.new("ShaderNodeMath")
    math_sign_times_2pi.name = "Math Sign Times 2Pi"
    math_sign_times_2pi.label = "sign(Δ) * 2π"
    math_sign_times_2pi.location = (150.0, -300.0)
    math_sign_times_2pi.bl_label = "Math"
    math_sign_times_2pi.operation = "MULTIPLY"
    math_sign_times_2pi.use_clamp = False
    math_sign_times_2pi.inputs[1].default_value = 6.2831854820251465
    links.new(math_sign.outputs[0], math_sign_times_2pi.inputs[0])

    # Step 9: long_delta = sign(Δ) * 2π - Δ
    math_long_delta = nodes.new("ShaderNodeMath")
    math_long_delta.name = "Math Long Delta"
    math_long_delta.label = "long_delta"
    math_long_delta.location = (350.0, -300.0)
    math_long_delta.bl_label = "Math"
    math_long_delta.operation = "SUBTRACT"
    math_long_delta.use_clamp = False
    links.new(math_sign_times_2pi.outputs[0], math_long_delta.inputs[0])
    links.new(math_sub_pi.outputs[0], math_long_delta.inputs[1])

    # Step 10: long_delta * factor
    math_scaled = nodes.new("ShaderNodeMath")
    math_scaled.name = "Math Scaled"
    math_scaled.label = "long_delta * factor"
    math_scaled.location = (550.0, -150.0)
    math_scaled.bl_label = "Math"
    math_scaled.operation = "MULTIPLY"
    math_scaled.use_clamp = False
    links.new(math_long_delta.outputs[0], math_scaled.inputs[0])
    links.new(group_input.outputs[0], math_scaled.inputs[1])  # Factor

    # Step 11: result = θ_B + (long_delta * factor)
    math_result = nodes.new("ShaderNodeMath")
    math_result.name = "Math Result"
    math_result.label = "θ_B + scaled"
    math_result.location = (600.0, 0.0)
    math_result.bl_label = "Math"
    math_result.operation = "ADD"
    math_result.use_clamp = False
    links.new(sample_index_b.outputs[0], math_result.inputs[0])
    links.new(math_scaled.outputs[0], math_result.inputs[1])
    links.new(math_result.outputs[0], group_output.inputs[0])

    auto_layout_nodes(group)
    return group

@geo_node_group
def create_mix_angle_linear_group():
    """Linear angle interpolation WITHOUT wrapping.
    Use this when you need all points to interpolate in the same direction.
    result = AngleA + (AngleB - AngleA) * factor
    """
    group_name = "MixAngleLinear"
    if group_name in bpy.data.node_groups:
        return bpy.data.node_groups[group_name]

    group = bpy.data.node_groups.new(group_name, "GeometryNodeTree")

    # --- Interface (same as MixAngle) ---
    socket = group.interface.new_socket(name="Result", in_out="OUTPUT", socket_type="NodeSocketFloat")
    socket.min_value = -3.4028234663852886e+38
    socket.max_value = 3.4028234663852886e+38
    socket = group.interface.new_socket(name="Factor", in_out="INPUT", socket_type="NodeSocketFloat")
    ...
    sample_index_a = nodes.new("GeometryNodeSampleIndex")
    sample_index_a.name = "Sample Index A"
    sample_index_a.label = "Sample AngleA"
    sample_index_a.location = (-550.0, 100.0)
    sample_index_a.bl_label = "Sample Index"
    sample_index_a.data_type = "FLOAT"
    sample_index_a.domain = "POINT"
    sample_index_a.clamp = False
    ...
    sample_index_b = nodes.new("GeometryNodeSampleIndex")
    sample_index_b.name = "Sample Index B"
    sample_index_b.label = "Sample AngleB"
    sample_index_b.location = (-550.0, -100.0)
    sample_index_b.bl_label = "Sample Index"
    sample_index_b.data_type = "FLOAT"
    sample_index_b.domain = "POINT"
    sample_index_b.clamp = False
    ...
    # Step 1: delta = AngleB - AngleA (NO wrapping!)
    math_delta = nodes.new("ShaderNodeMath")
    math_delta.name = "Math Delta"
    math_delta.label = "B - A"
    math_delta.location = (-300.0, 0.0)
    math_delta.bl_label = "Math"
    math_delta.operation = "SUBTRACT"
    math_delta.use_clamp = False
    links.new(sample_index_b.outputs[0], math_delta.inputs[0])
    links.new(sample_index_a.outputs[0], math_delta.inputs[1])

    # Step 2: scaled = delta * factor
    math_scaled = nodes.new("ShaderNodeMath")
    math_scaled.name = "Math Scaled"
    math_scaled.label = "delta * factor"
    math_scaled.location = (-50.0, 0.0)
    math_scaled.bl_label = "Math"
    math_scaled.operation = "MULTIPLY"
    math_scaled.use_clamp = False
    links.new(math_delta.outputs[0], math_scaled.inputs[0])
    links.new(group_input.outputs[0], math_scaled.inputs[1])  # Factor

    # Step 3: result = AngleA + scaled
    math_result = nodes.new("ShaderNodeMath")
    math_result.name = "Math Result"
    math_result.label = "A + scaled"
    math_result.location = (200.0, 0.0)
    math_result.bl_label = "Math"
    math_result.operation = "ADD"
    math_result.use_clamp = False
    links.new(sample_index_a.outputs[0], math_result.inputs[0])
    links.new(math_scaled.outputs[0], math_result.inputs[1])
    links.new(math_result.outputs[0], group_output.inputs[0])
...
def create_male_torso__loft_0__group_group():
    ...
    math_001.name = "Math.001"
    math_001.label = ""
    math_001.location = (552.1984252929688, -712.4992065429688)
    math_001.bl_label = "Math"
    math_001.operation = "MULTIPLY"
    math_001.use_clamp = False
    # Value
    ...
    math_001.inputs[2].default_value = 0.5
    # Links for math_001
    links.new(spherical_coords.outputs[2], math_001.inputs[0])
