...
def __getattr__(name):
    ...
    if name in ('DualRadial', 'QuadRadial', 'IKLimits', 'RadialAttachment', 
                'Joint', 'SegmentChain', 'JoinedStructure', 'AttachedStructure',
    ...
    if name in ('DSLExecutor', 'DSLExecutionResult', 'DSLInstance',
                'execute_dsl_file', 'get_dsl_instances', 'get_dsl_definitions'):
        from procedural_human.dsl import executor
        return getattr(executor, name)
    
    if name in ('DSLGenerator', 'GeneratedObject', 
                'generate_from_dsl_file', 'regenerate_dsl_object'):
