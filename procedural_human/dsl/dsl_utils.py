    ...
    for vert in mesh.vertices:
        ...
        for z_start, z_end, idx in z_positions:
            ...
            if z_start <= v_z <= z_end:
                local_pos = (
                    (v_z - z_start) / (z_end - z_start) if z_end > z_start else 0.5
                )
...
def get_bone_info_from_object(obj: Any) -> List[Dict]:
    ...
    logger.info(
        f"[Bone Debug] Getting bone info for: source={source_file}, instance={instance_name}"
    )
    ...
    if instance_name not in result.instances:
        logger.info(
            f"[Bone Debug] Instance '{instance_name}' not in result.instances: {list(result.instances.keys())}"
        )
        return []
    ...
    if bones_list is None:
        ...
        logger.info(
            f"[Bone Debug] Available attributes: {[a for a in dir(instance) if not a.startswith('__') and not callable(getattr(instance, a, None))]}"
        )
        return []
    ...
    for idx, bone in enumerate(bones_list):
        ...
        logger.info(
            f"[Bone Debug] Bone {idx}: length={info['length']}, ik={info['ik_limits']}"
        )
