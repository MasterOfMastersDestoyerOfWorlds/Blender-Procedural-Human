...
def create_hex_profile_spheroid_group(suffix=None):
    ...
    # Inputs
    group.interface.new_socket(name="Radius", in_out="INPUT", socket_type="NodeSocketFloat").default_value = 1.0
    
    # 6 Curve Inputs
    directions = ["X+", "X-", "Y+", "Y-", "Z+", "Z-"]
    for d in directions:
        group.interface.new_socket(name=f"{d} Float Curve", in_out="INPUT", socket_type="NodeSocketClosure")
    ...
    def create_axis_logic(axis_char, comp_node, pos_socket, neg_socket, param_input_x, param_input_y):
        """
    ...
    # Logic for X
    x_sq = create_axis_logic('X', sep_xyz.outputs["X"], 
                             input_node.outputs["X+ Float Curve"], input_node.outputs["X- Float Curve"],
                             sep_xyz.outputs["Y"], sep_xyz.outputs["Z"])
                             
    # Logic for Y
    y_sq = create_axis_logic('Y', sep_xyz.outputs["Y"], 
                             input_node.outputs["Y+ Float Curve"], input_node.outputs["Y- Float Curve"],
                             sep_xyz.outputs["X"], sep_xyz.outputs["Z"])
                             
    # Logic for Z
    z_sq = create_axis_logic('Z', sep_xyz.outputs["Z"], 
                             input_node.outputs["Z+ Float Curve"], input_node.outputs["Z- Float Curve"],
                             sep_xyz.outputs["X"], sep_xyz.outputs["Y"])
