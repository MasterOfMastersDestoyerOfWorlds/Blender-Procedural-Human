...
import bpy
from procedural_human.dsl.primitives.dual_radial.dual_radial_nodes import (
...
class DualRadial:
    ...
    def _create_segment_node_group(
        ...
        segment_group.links.new(
            input_node.outputs["Segment Radius"], radial_instance.inputs["Radius"]
        )
        segment_group.links.new(
            separate_xyz.outputs["Z"], radial_instance.inputs["Z Position"]
        )
        segment_group.links.new(
        ...
        segment_group.links.new(
            input_node.outputs["X Float Curve"], radial_instance.inputs["X Float Curve"]
        )
        segment_group.links.new(
            input_node.outputs["Y Float Curve"], radial_instance.inputs["Y Float Curve"]
        )
        segment_group.links.new(
            input_node.outputs["Sample Count"], grid.inputs["Vertices Y"]
        )
        ...
        segment_group.links.new(
            input_node.outputs["Segment Radius"], output_node.inputs["Segment Radius"]
        )
    ...
    def _apply_preset_to_closures(
        ...
        from procedural_human.utils.curve_serialization import (
            apply_data_to_float_curve_node,
        )
        ...
        logger.info(
            f"[Preset Debug] Found preset '{preset_name}' with keys: {list(preset_data.keys())}"
        )
        ...
        if x_key in preset_data and x_closure.curve_node:
            ...
            logger.info(
                f"[Preset Debug] Key '{x_key}' not in preset or closure missing"
            )
        ...
        if y_key in preset_data and y_closure.curve_node:
            ...
            logger.info(
                f"[Preset Debug] Key '{y_key}' not in preset or closure missing"
            )
