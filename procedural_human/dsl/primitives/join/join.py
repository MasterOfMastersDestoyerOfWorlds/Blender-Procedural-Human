...
class Joint:
    ...
    def generate(
        ...
        joint_instance_x = (
            first_closure.out_node.location[0] + first_closure.out_node.width + 100
        )
        ...
        for closure, input_name in zip(closures, closure_inputs):
            node_group.links.new(
                closure.output_socket, joint_instance.inputs[input_name]
            )
    ...
    def _apply_preset_to_joint_closures(
        ...
        for closure, angle_label in zip(closures, curve_labels):
            ...
            logger.info(
                f"[Preset Debug] Checking key '{key}' for closure with label '{closure.label if hasattr(closure, 'label') else 'unknown'}'"
            )
            ...
            if key in preset_data:
                ...
                if closure.curve_node:
                    ...
                    if apply_data_to_float_curve_node(
                        closure.curve_node, preset_data[key]
                    ):
                        applied = True
                        logger.info(f"[Preset Debug] Successfully applied {key}")
                    else:
                        logger.info(
                            f"[Preset Debug] Failed to apply {key} to curve node"
                        )
                else:
...
class JoinedStructure:
    ...
    segment_source: Any = (
        None  # Original SegmentChain if passed, for dependency tracking
    )
    ...
    def generate(self, context: GenerationContext, segment_results: List[Dict]) -> Dict:
        ...
        assert len(joint_results) == len(
            self.joints
        ), f"Number of joint results: {len(joint_results)} does not match number of joints: {len(self.joints)}"
        return {
