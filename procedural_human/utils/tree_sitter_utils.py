            ...
            if preset_name:
                ...
                if i < len(preset_name_nodes):
                    preset_name_str = (
                        source_bytes[
                            preset_name_nodes[i]
                            .start_byte : preset_name_nodes[i]
                            .end_byte
                        ]
                        .decode("utf-8")
                        .strip("\"'")
...
def replace_get_data_method(file_path: str, preset_name: str, new_data: dict) -> bool:
    """
    ...
    if return_line_idx is None:
        ...
        if method_body_start < len(lines):
            indent = lines[method_body_start][
    ...
    for body_child in class_body.children:
        if body_child.type == "function_definition":
            ...
            if func_name == "__init__":
                if func_params:
                    class_info["init_params"] = _extract_params(
                        func_params, source_bytes
                    )
                if func_body:
                    class_info["components"] = _extract_components(
                        func_body, source_bytes
                    )
                break
...
def _parse_component_assignment(
    assign_node: Any, source_bytes: bytes
) -> Optional[Dict]:
        ...
        if child.type == "block":
            for body_child in child.children:
                if body_child.type == "expression_statement":
                    ...
                    if expr and expr.type == "assignment":
                        component_info = _parse_indexed_component(
                            expr, source_bytes, loop_var
                        )
                        if component_info:
...
def _parse_indexed_component(
    assign_node: Any, source_bytes: bytes, loop_var: str
) -> Optional[Dict]:
...
def extract_instance_assignments(file_path: str) -> List[Dict]:
    ...
    for child in root_node.children:
        if child.type == "expression_statement":
            expr = child.children[0] if child.children else None
            if expr and expr.type == "assignment":
                instance_info = _parse_instance_assignment(
                    expr, source_bytes, class_names
                )
                if instance_info:
...
def _parse_instance_assignment(
    assign_node: Any, source_bytes: bytes, class_names: set
) -> Optional[Dict]:
    ...
    if last_import_end > 0:
        ...
        if source_str.startswith('"""') or source_str.startswith("'''"):
            ...
            if end_docstring != -1:
                end_docstring += 3
                while (
                    end_docstring < len(source_str)
                    and source_str[end_docstring] in "\n\r"
                ):
                    end_docstring += 1
                new_content = (
    ...
    for preset_name, existing_range in existing_presets.items():
        if existing_range:
            ranges_to_remove.append(
                (existing_range["start_line"] - 1, existing_range["end_line"])
            )
...
'''
    ...
    try:
        ...
        logger.info(
            f"[TreeSitter] Warning: Generated code has syntax error at line {e.lineno}: {e.msg}"
        )
        logger.info(f"[TreeSitter] Skipping write to {file_path}")
        return False
